...
import { HeatingVolumeInputs, CalculationResult } from '../types';
import { KTP_CONSTANT, ALPHA_TABLE, DEFAULT_HEATING_VOLUME_INPUTS } from '../constants';
import { Q0_DATA, TO_DATA, TI_DATA } from '../q0Data';
import { InputField } from './InputField';

interface HeatingVolumeCalculatorProps {
  inputs: HeatingVolumeInputs;
  setInputs: React.Dispatch<React.SetStateAction<HeatingVolumeInputs>>;
  onBack: () => void;
}
...
export const HeatingVolumeCalculator: React.FC<HeatingVolumeCalculatorProps> = ({ 
  ...
  const handleReset = () => {
    if (confirm('Сбросить все данные до значений по умолчанию?')) {
      setInputs(DEFAULT_HEATING_VOLUME_INPUTS);
    }
  };
